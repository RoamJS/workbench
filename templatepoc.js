...
window.getTemplate = function(name) {
  ...
  function resolveNode(id, level, trail, resolve, skipFirstPrefix, stop) {
    ...
    var prefix = skipFirstPrefix
      ? ""
      : " ".repeat(2 * Math.max(level - 1, 0)) + "- "; // indention starting from level 2
    ...
    if (typeof node[":block/string"] != "undefined") {
      ...
      var regexEmbed = resolve
        ? /!?{{\[*embed\]*\s*:\s*\(\(([^\)]*)\)\)\s*}}/gi
        : /!{{\[*embed\]*\s*:\s*\(\(([^\)]*)\)\)\s*}}/gi;
      node[":block/string"] = node[":block/string"].replace(
        regexEmbed,
        function(_, v) {
          var uid = v.trim();
          var id = window.roamAlphaAPI.q(
          ...
          if (typeof block != "undefined") {
            // for loops we got back undefined
            return block;
          } else {
            return "LOOP:" + _;
          }
...
window.KeyboardLib = {
  ...
  async simulateSequence(events, delayOverride) {
    events.forEach(function(e) {
      return KeyboardLib.getActiveEditElement().dispatchEvent(
        KeyboardLib.getKeyboardEvent(e.name, e.code, e.opt)
      );
    });
  ...
  async simulateKey(code, delayOverride, opts) {
    return this.simulateSequence(
      [
        { name: "keydown", code: code, opt: opts },
        { name: "keyup", code: code, opt: opts }
      ],
      delayOverride
    );
  },
