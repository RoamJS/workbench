...
export const state = get("LivePreview ") || "on";
roam42.livePreview.browserHeight =
  (await roam42.settings.get("LivePreviewHeight")) || "500px";
roam42.livePreview.browserWidth =
  (await roam42.settings.get("LivePreviewWidth")) || "500px";
roam42.livePreview.delay = Number(
  (await roam42.settings.get("LivePreviewDelay")) || "100"
);
...
if (window === window.parent) {
  ...
  const createPreviewIframe = () => {
    ...
    if (roam42.livePreview.state == "optimized") url = url + "?disablejs=true";
  ...
  const enableLivePreview = () => {
    ...
    roam42.livePreview.roam42LivePreviewState =
      roam42.livePreview.getRoamLivePreviewState(); //get current state of live preview
    ...
    document.addEventListener("mouseover", (e) => {
      ...
      if (
        isPageRef == false &&
        (e.ctrlKey == true || roam42.livePreview.roam42LivePreviewState == 2) &&
